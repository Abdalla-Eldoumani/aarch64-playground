...
describe("line map duplicate and gap handling", () => {
  it("keeps the last line seen for a repeated instruction address", () => {
    // The same address mapped twice (14 then 22): addrToLine takes the later
    // pair, while both editor lines still resolve back to the address.
    const map = parseLineMap([CODE_BASE, 14, CODE_BASE, 22]);
    expect(map.addrToLine.get(CODE_BASE)).toBe(22);
    expect(map.lineToAddr.get(14)).toBe(CODE_BASE);
    expect(map.lineToAddr.get(22)).toBe(CODE_BASE);
  });

  it("resolves a gap line to the nearest following mapped line, not just any later one", () => {
    const map = parseLineMap([
      CODE_BASE, 5,
      CODE_BASE + 4, 9,
      CODE_BASE + 8, 14,
    ]);
    expect(lineToAddrFromMap(6, map)).toBe(CODE_BASE + 4);
    expect(lineToAddrFromMap(10, map)).toBe(CODE_BASE + 8);
  });

  it("resolves a line before the first mapped line to the first instruction", () => {
    const map = parseLineMap(COMPLEX_FLAT);
    expect(lineToAddrFromMap(0, map)).toBe(CODE_BASE);
    expect(lineToAddrFromMap(1, map)).toBe(CODE_BASE);
  });
});
