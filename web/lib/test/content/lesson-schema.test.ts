...
describe("validateLesson (message quality on malformed shapes)", () => {
  test("rejects a whitespace-only title", () => {
    expect(rejectError({ ...validLesson(), title: "   " })).toMatch(/title/);
  });

  test("rejects a boolean order", () => {
    expect(rejectError({ ...validLesson(), order: true })).toMatch(/order/);
  });

  test("rejects a non-string summary when present", () => {
  ...
  test("rejects a body entry that is not an object, naming its index", () => {
    expect(rejectError({ ...validLesson(), body: [null] })).toMatch(/body\[0\].*block object/);
    expect(rejectError({ ...validLesson(), body: [["prose"]] })).toMatch(/body\[0\]/);
    expect(rejectError({ ...validLesson(), body: ["prose"] })).toMatch(/body\[0\]/);
  });

  test("rejects a block without a type tag", () => {
    expect(rejectError({ ...validLesson(), body: [{ markdown: "x" }] })).toMatch(
      /body\[0\].*missing block type/,
    );
  });

  test("names the unknown block type in the error", () => {
    expect(rejectError({ ...validLesson(), body: [{ type: "video", src: "x" }] })).toMatch(
      /unknown block type "video"/,
    );
  });

  test("rejects a prose block whose markdown is not a string, naming block and field", () => {
    const error = rejectError({ ...validLesson(), body: [{ type: "prose", markdown: 7 }] });
    expect(error).toMatch(/body\[0\] \(prose\)/);
    expect(error).toMatch(/markdown/);
  });

  test("rejects editor args and stdin that are present but not strings", () => {
    expect(
      rejectError({ ...validLesson(), body: [{ type: "editor", starter: "", args: 1 }] }),
    ).toMatch(/args/);
    expect(
      rejectError({ ...validLesson(), body: [{ type: "editor", starter: "", stdin: 1 }] }),
    ).toMatch(/stdin/);
  });

  test("keeps editor args and stdin when they are valid strings", () => {
    const result = validateLesson({
      ...validLesson(),
      body: [{ type: "editor", starter: "mov x0, 1\n", args: "2 3", stdin: "9\n" }],
    });
    expect(result.ok).toBe(true);
    if (!result.ok) throw new Error(result.error);
  ...
  test("omits editor args and stdin entirely when absent", () => {
    const result = validateLesson({
      ...validLesson(),
      body: [{ type: "editor", starter: "" }],
    });
    expect(result.ok).toBe(true);
    if (!result.ok) throw new Error(result.error);
