...
describe("LectureBar", () => {
  it("renders a step and reset button", () => {
    render(<LectureBar onStep={vi.fn()} onReset={vi.fn()} stepCount={0} isHalted={false} />);
  ...
  it("invokes onStep when the step button is clicked", () => {
    const onStep = vi.fn();
    render(<LectureBar onStep={onStep} onReset={vi.fn()} stepCount={0} isHalted={false} />);
    fireEvent.click(screen.getByRole("button", { name: /step/i }));
    expect(onStep).toHaveBeenCalledTimes(1);
  });

  it("re-applies the step-pop animation when stepCount changes", () => {
    const { rerender } = render(<LectureBar onStep={vi.fn()} onReset={vi.fn()} stepCount={0} isHalted={false} />);
    const before = screen.getByRole("button", { name: /step/i });
    rerender(<LectureBar onStep={vi.fn()} onReset={vi.fn()} stepCount={1} isHalted={false} />);
  ...
  it("disables the step button when halted", () => {
    render(<LectureBar onStep={vi.fn()} onReset={vi.fn()} stepCount={0} isHalted={true} />);
