...
// EmbeddablePlayground pulls in Monaco + WASM + Worker; rendering it in
// jsdom blows up before we get to the markup we care about. Instead this
// test pins the contract that the embed-mode CSS rule keys off:
//   - EmbeddablePlayground marks the source GitHub link with data-embed-hide="1"
...
describe("embed mode markup contract", () => {
  it("EmbeddablePlayground marks the source link with data-embed-hide", () => {
    const file = readFileSync(
      path.join(__dirname, "..", "components", "EmbeddablePlayground.tsx"),
      "utf8",
    );
