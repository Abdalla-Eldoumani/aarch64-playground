...
The idea is simple and it earns its keep: with a reasonably functional
libc, gcc can generate an endless supply of test programs, and a real
machine can say what each one must do. The first sweep of this corpus
...
- `NAME.O2.s` / `NAME.O2.out` / `NAME.O2.code` -- the `-O2` tier, run as
  an ignored coverage map (`cargo test --test c_corpus -- --ignored`),
...
The playground's named messages are the intended improvement over the
server's bare `Segmentation fault`.
